...
Strapping.prototype.initialize = function (options) {
  ...
  this.addedFonts = [];
  if (!options.variables) this.load(themes.filter(function (t) {
...
templates.strapping = function (opts) {
  ...
  var links = '\n    <label>Jump to:</label><br>\n  ' + inputGroups.concat([{ label: "Miscellaneous" }]).map(function (g) {
  ...
  inputGroups.forEach(function (g) {
    var matchingInputs = Object.keys(opts.vars).filter(function (k) {
      return k.substring(1).match(g.pattern);
    });
    addedInputs = addedInputs.concat(matchingInputs);
    var inputGroupHTML = '<a name="' + g.label + '"></a><h2>' + g.label + '</h2>';
  ...
  inputs += '<a name="Miscellaneous"></a><h2>Miscellaneous</h2>' + unmatchedInputs.map(function (k) {
    return templates.input(k, opts.vars[k]);
  }).join('\n');
