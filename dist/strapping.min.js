...
utils.unescapeQuotes = function (str) {
  return str.replace(/&quot;/g, '"');
};

utils.isColor = function (name) {
  return name.match(/(brand|color|gray|-bg$|-text$|-border$)/);
};

utils.addCSS = function (css, id) {
...
utils.removeCSS = function (id) {
  var elem = document.getElementById(id);
  if (elem) elem.parentNode.removeChild(elem);
};

utils.replaceLessVars = function (less) {
  var vars = {};
  for (var key in less) {
    var val = less[key];
...
utils.getSassFromVariables = function (variables) {
  var varFile = '$bootstrap-sass-asset-helper: false !default;\n';
  varFile += Object.keys(variables).map(function (v) {
    return v + ': ' + variables[v] + ' !default;';
  }).join('\n');
  return varFile;
};

utils.getVariablesFromSass = function (scss) {
  var vars = {};
  scss.split('\n').map(function (l) {
    return l.match(/^(\$\S+):\s*(.*)\s*!default;.*$/);
  }).filter(function (l) {
    return l;
  }).map(function (match) {
...
utils.getSassFromFonts = function (fonts) {
  return fonts.map(function (font) {
    return ('\n@import url(\'https://fonts.googleapis.com/css?family=' + font.family + ':' + font.variants.join(",") + '\');\n  ').trim();
  }).join('\n');
};

utils.getFontsFromSass = function (scss) {
  var lines = scss.split('\n');
  var fontLines = lines.map(function (l) {
    return l.match(/^@import url\(.*fonts.googleapis.com.css.family=([^:]+):(.*)['"]\)/);
  }).filter(function (m) {
    return m;
  });
  return fontLines.map(function (l) {
    return l[1];
  });
};
...
var Sass = __webpack_require__(22);
var ColorPicker = __webpack_require__(23);
var saveAs = __webpack_require__(14).saveAs;

var SASS_FILES = __webpack_require__(5);
var utils = __webpack_require__(1);
var templates = __webpack_require__(7);
var themes = __webpack_require__(0);

var Strapping = module.exports = function () {};
...
Strapping.prototype.initialize = function (options) {
  ...
  Object.keys(SASS_FILES).forEach(function (filename) {
    _this.sass.writeFile(filename, SASS_FILES[filename]);
  });
  this.heading = options.heading;
  if (!window.Strapping.initialized) {
    utils.addCSS(__webpack_require__(118));
    utils.addCSS(__webpack_require__(28));
  }
  ...
  this.loadFonts(function () {
    if (!options.variables) _this.load(themes.filter(function (t) {
      return t.name === 'Strapping';
    })[0].scss, true);
    _this.compile();
  });
};
...
Strapping.prototype.saveAs = function (type) {
  ...
  this.compile(function (result) {
    ...
    var text = type === 'json' ? JSON.stringify(result.variables, null, 2) : result[type];
    var blob = new Blob([text], { type: "text/plain;charset=utf-8" });
    var filename = '';
    if (type === 'json') filename = 'variables.json';else if (type === 'css') filename = 'bootstrap.css';else if (type === 'sass') filename = '_variables.scss';
...
Strapping.prototype.load = function (str, noCompile) {
  ...
  if (jsonObj) {
    if (jsonObj.vars) vars = utils.replaceLessVars(jsonObj.vars);else if (jsonObj.bootstrap && jsonObj.bootstrap.vars) vars = utils.replaceLessVars(jsonObj.bootstrap.vars);
    if (jsonObj.fonts) {
      this.addedFonts = [];
      Object.keys(jsonObj.fonts).forEach(function (fam) {
        return _this2.addFont(fam);
      });
    }
  } else if (!vars) {
    vars = utils.getVariablesFromSass(str);
    this.addedFonts = [];
    var sassFonts = utils.getFontsFromSass(str);
    sassFonts.forEach(function (f) {
      return _this2.addFont(f);
    });
  }
...
Strapping.prototype.setTheme = function (themeName) {
  ...
  var vars = utils.getVariablesFromSass(theme.scss);
  for (var key in vars) {
    this.variables[key] = vars[key];
  }
...
Strapping.prototype.drawEditor = function (err) {
  ...
  var varsCSS = Object.keys(this.variables).filter(function (v) {
    return utils.isColor(v);
  }).map(function (v) {
    return '\n#StrappingEditor input[name="' + utils.escapeQuotes(v) + '"] ~ .input-group-addon {\nbackground-color: ' + _this3.variables[v] + ';\n}\n  ';
  }).join('\n');
  varsCSS = '@import "bootstrap/_variables.scss";\n' + varsCSS;
  this.sass.compile(varsCSS, function (result) {
...
Strapping.prototype.compile = function (callback, skipInputs) {
  ...
  if (!skipInputs && this.compiledOnce) {
    Object.keys(this.variables).forEach(function (v) {
      _this4.variables[v] = utils.unescapeQuotes(document.getElementsByName(v)[0].value);
    });
  }
  var varFile = utils.getSassFromFonts(this.addedFonts) + '\n' + utils.getSassFromVariables(this.variables);
  this.sass.writeFile('bootstrap/_variables.scss', varFile);
  this.editor.innerHTML = templates.loading();
  this.sass.compile('@import "_bootstrap";', function (result) {
    utils.removeCSS('bootstrap');
    utils.removeCSS('strapping');
    utils.addCSS(result.text, 'bootstrap');
    _this4.drawEditor(result.status ? result.message : null);
    _this4.compiledOnce = true;
    callback({
      status: result.status,
      message: result.message,
      sass: varFile,
      css: result.text,
      variables: _this4.variables
    });
  });
};
...
Strapping.prototype.showColorPicker = function (elem) {
  ...
  this.picker.onChange(function (color) {
    if (_this5.picker.init) {
      input.value = color.toLowerCase();
      elem.querySelector('.input-group-addon').setAttribute('style', 'background-color: ' + color);
    }
    _this5.picker.init = true;
  });
  input.addEventListener('change', function (evt) {
    var color = evt.target.value;
    if (color && color !== _this5.picker.color && color.match(/^#\w+/)) {
      _this5.picker.setColor(color);
    }
  });
};

var FONTS_URL = "https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyCIPThYcVkXtTiNbDPBD1FdvKbPNX7d3iw";
...
Strapping.prototype.loadFonts = function (cb) {
  ...
  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
      _this6.fonts = JSON.parse(xmlHttp.responseText).items;
      cb();
    }
  };
...
Strapping.prototype.addFont = function (family) {
  var newFont = this.fonts.filter(function (f) {
...
Strapping.prototype.removeFont = function (family) {
  this.addedFonts = this.addedFonts.filter(function (f) {
    return f.family !== family;
  });
  this.drawEditor();
};
...
module.exports = __webpack_require__.p + "static/index.html";
...
window.BOOTSTRAP_FILES = module.exports = {
  'bootstrap/_dropdowns.scss': __webpack_require__(42),
  'bootstrap/_navs.scss': __webpack_require__(54),
  'bootstrap/_panels.scss': __webpack_require__(58),
  'bootstrap/_type.scss': __webpack_require__(69),
  'bootstrap/_close.scss': __webpack_require__(39),
  'bootstrap/_modals.scss': __webpack_require__(52),
  'bootstrap/_forms.scss': __webpack_require__(43),
  'bootstrap/_media.scss': __webpack_require__(50),
  'bootstrap/_labels.scss': __webpack_require__(48),
  'bootstrap/_component-animations.scss': __webpack_require__(41),
  'bootstrap/_responsive-embed.scss': __webpack_require__(62),
  'bootstrap/_breadcrumbs.scss': __webpack_require__(35),
  'bootstrap/_thumbnails.scss': __webpack_require__(67),
  'bootstrap/_grid.scss': __webpack_require__(45),
  'bootstrap/mixins/_text-emphasis.scss': __webpack_require__(99),
  'bootstrap/mixins/_size.scss': __webpack_require__(96),
  'bootstrap/mixins/_panels.scss': __webpack_require__(90),
  'bootstrap/mixins/_reset-text.scss': __webpack_require__(93),
  'bootstrap/mixins/_reset-filter.scss': __webpack_require__(92),
  'bootstrap/mixins/_vendor-prefixes.scss': __webpack_require__(101),
  'bootstrap/mixins/_forms.scss': __webpack_require__(78),
  'bootstrap/mixins/_labels.scss': __webpack_require__(84),
  'bootstrap/mixins/_nav-vertical-align.scss': __webpack_require__(87),
  'bootstrap/mixins/_border-radius.scss': __webpack_require__(74),
  'bootstrap/mixins/_grid.scss': __webpack_require__(81),
  'bootstrap/mixins/_center-block.scss': __webpack_require__(76),
  'bootstrap/mixins/_grid-framework.scss': __webpack_require__(80),
  'bootstrap/mixins/_gradients.scss': __webpack_require__(79),
  'bootstrap/mixins/_buttons.scss': __webpack_require__(75),
  'bootstrap/mixins/_pagination.scss': __webpack_require__(89),
  'bootstrap/mixins/_tab-focus.scss': __webpack_require__(97),
  'bootstrap/mixins/_text-overflow.scss': __webpack_require__(100),
  'bootstrap/mixins/_resize.scss': __webpack_require__(94),
  'bootstrap/mixins/_image.scss': __webpack_require__(83),
  'bootstrap/mixins/_clearfix.scss': __webpack_require__(77),
  'bootstrap/mixins/_alerts.scss': __webpack_require__(72),
  'bootstrap/mixins/_background-variant.scss': __webpack_require__(73),
  'bootstrap/mixins/_nav-divider.scss': __webpack_require__(86),
  'bootstrap/mixins/_opacity.scss': __webpack_require__(88),
  'bootstrap/mixins/_list-group.scss': __webpack_require__(85),
  'bootstrap/mixins/_responsive-visibility.scss': __webpack_require__(95),
  'bootstrap/mixins/_table-row.scss': __webpack_require__(98),
  'bootstrap/mixins/_hide-text.scss': __webpack_require__(82),
  'bootstrap/mixins/_progress-bar.scss': __webpack_require__(91),
  'bootstrap/_tooltip.scss': __webpack_require__(68),
  'bootstrap/_buttons.scss': __webpack_require__(37),
  'bootstrap/_button-groups.scss': __webpack_require__(36),
  'bootstrap/_navbar.scss': __webpack_require__(53),
  'bootstrap/_pagination.scss': __webpack_require__(57),
  'bootstrap/_theme.scss': __webpack_require__(66),
  'bootstrap/_input-groups.scss': __webpack_require__(46),
  'bootstrap/_badges.scss': __webpack_require__(34),
  'bootstrap/_utilities.scss': __webpack_require__(70),
  'bootstrap/_jumbotron.scss': __webpack_require__(47),
  'bootstrap/_popovers.scss': __webpack_require__(59),
  'bootstrap/_progress-bars.scss': __webpack_require__(61),
  'bootstrap/_code.scss': __webpack_require__(40),
  'bootstrap/_scaffolding.scss': __webpack_require__(64),
  'bootstrap/_print.scss': __webpack_require__(60),
  'bootstrap/_alerts.scss': __webpack_require__(33),
  'bootstrap/_variables.scss': __webpack_require__(2),
  'bootstrap/_pager.scss': __webpack_require__(56),
  'bootstrap/_responsive-utilities.scss': __webpack_require__(63),
  'bootstrap/_normalize.scss': __webpack_require__(55),
  'bootstrap/_list-group.scss': __webpack_require__(49),
  'bootstrap/_tables.scss': __webpack_require__(65),
  'bootstrap/_glyphicons.scss': __webpack_require__(44),
  'bootstrap/_mixins.scss': __webpack_require__(51),
  'bootstrap/_wells.scss': __webpack_require__(71),
  'bootstrap/_carousel.scss': __webpack_require__(38),
  '_bootstrap-mincer.scss': __webpack_require__(30),
  '_bootstrap.scss': __webpack_require__(32),
  '_bootstrap-compass.scss': __webpack_require__(29),
  '_bootstrap-sprockets.scss': __webpack_require__(31)
};
...
var INPUT_GROUPS = module.exports = [{
  label: "Fonts",
...
var inputGroups = __webpack_require__(6);
var utils = __webpack_require__(1);
var themes = __webpack_require__(0);
...
templates.fonts = function (fonts, addedFonts) {
  var fontItem = function fontItem(font) {
    return '<li><a onclick="strapping.addFont(\'' + font.family + '\')">' + font.family + '</a></li>';
  };
  var addedFontItem = function addedFontItem(font) {
  ...
  return '\n<div class="dropdown form-group">\n  <a class="btn btn-default dropdown-toggle" data-toggle="dropdown">Add Fonts <span class="caret"></span></a>\n  <ul class="dropdown-menu">\n    ' + fonts.map(function (f) {
    return fontItem(f);
  }).join('\n') + '\n  </ul>\n</div>\n<p>Preview fonts on <a href="https://fonts.google.com/" target="_blank">fonts.google.com</a></p>\n<ul>\n  ' + addedFonts.map(function (f) {
    return addedFontItem(f);
  }).join('\n') + '\n</ul>\n  ';
};

var inputMatchesFields = function inputMatchesFields(name, fields) {
  if (!fields) return true;
  var matches = fields.filter(function (pattern) {
    if (typeof pattern === 'string') return name === pattern;else return name.match(pattern);
  });
  return !!matches.length;
};

templates.strapping = function (opts) {
  var heading = opts.heading || templates.heading();

  var error = opts.error ? '<div class="alert alert-warning">' + opts.error + '</div>' : '';

  var addedInputs = [];
  var addedInputGroups = [];
  var inputs = '';
  inputGroups.forEach(function (group) {
    var matchingInputs = Object.keys(opts.vars).filter(function (k) {
      return k.substring(1).match(group.pattern);
    }).filter(function (k) {
      return inputMatchesFields(k.substring(1), opts.fields);
    });
    if (!matchingInputs.length) return;
    addedInputs = addedInputs.concat(matchingInputs);
    addedInputGroups.push(group);
    var inputGroupHTML = '<a name="' + group.label + '"></a><h2>' + group.label + '</h2>';
    if (group.label === 'Fonts') inputGroupHTML += templates.fonts(opts.fonts, opts.addedFonts);
    inputGroupHTML += matchingInputs.map(function (k) {
      return templates.input(k, opts.vars[k]);
    }).join('\n');
    inputs += inputGroupHTML;
  });
  var unmatchedInputs = Object.keys(opts.vars).filter(function (k) {
    return addedInputs.indexOf(k) === -1;
  }).filter(function (k) {
    return inputMatchesFields(k.substring(1), opts.fields);
  });
  if (unmatchedInputs.length) {
    inputs += '<a name="Miscellaneous"></a><h2>Miscellaneous</h2>';
    inputs += unmatchedInputs.map(function (k) {
      return templates.input(k, opts.vars[k]);
    }).join('\n');
    addedInputGroups.push({ label: "Miscellaneous" });
  }

  var links = '\n    <label>Jump to:</label><br>\n  ' + addedInputGroups.map(function (g) {
...
var prefix = __webpack_require__(21);
var isArray = __webpack_require__(16);
var properties = __webpack_require__(13);
var applyDefaultUnit = __webpack_require__(12);
...
function transform(target, opts) {
  ...
  for (propName in opts) {
    ...
    if (_has.call(properties.transform, propName)) {
      ...
      transformOutput.push(propName + '(' + applyDefaultUnit(propValue, propData.defaultUnit, propData.separator) + ')');
    ...
    if (_has.call(properties, propName)) {
      ...
      target.style[prefix(propName)] = applyDefaultUnit(propValue, propData.defaultUnit, propData.separator);

      continue;
    }

    console.warn('dom-transform: this property (`' + propName + '`) is not supported.');
  }
...
module.exports = "// Reset filters for IE\n//\n// When you need to remove a gradient background, do not forget to use this to reset\n// the IE filter for IE9 and below.\n\n@mixin reset-filter() {\n  filter: progid:DXImageTransform.Microsoft.gradient(enabled = false);\n}\n"

/***/ }),
/* 93 */
/***/ (function(module, exports) {

module.exports = "@mixin reset-text() {\n  font-family: $font-family-base;\n  // We deliberately do NOT reset font-size.\n  font-style: normal;\n  font-weight: normal;\n  letter-spacing: normal;\n  line-break: auto;\n  line-height: $line-height-base;\n  text-align: left; // Fallback for where `start` is not supported\n  text-align: start;\n  text-decoration: none;\n  text-shadow: none;\n  text-transform: none;\n  white-space: normal;\n  word-break: normal;\n  word-spacing: normal;\n  word-wrap: normal;\n}\n"

/***/ }),
/* 94 */
/***/ (function(module, exports) {

module.exports = "// Resize anything\n\n@mixin resizable($direction) {\n  resize: $direction; // Options: horizontal, vertical, both\n  overflow: auto; // Per CSS3 UI, `resize` only applies when `overflow` isn't `visible`\n}\n"

/***/ }),
/* 95 */
/***/ (function(module, exports) {

module.exports = "// Responsive utilities\n\n//\n// More easily include all the states for responsive-utilities.less.\n// [converter] $parent hack\n@mixin responsive-visibility($parent) {\n  #{$parent} {\n    display: block !important;\n  }\n  table#{$parent}  { display: table !important; }\n  tr#{$parent}     { display: table-row !important; }\n  th#{$parent},\n  td#{$parent}     { display: table-cell !important; }\n}\n\n// [converter] $parent hack\n@mixin responsive-invisibility($parent) {\n  #{$parent} {\n    display: none !important;\n  }\n}\n"

/***/ }),
/* 96 */
/***/ (function(module, exports) {

module.exports = "// Sizing shortcuts\n\n@mixin size($width, $height) {\n  width: $width;\n  height: $height;\n}\n\n@mixin square($size) {\n  @include size($size, $size);\n}\n"

/***/ }),
/* 97 */
/***/ (function(module, exports) {

module.exports = "// WebKit-style focus\n\n@mixin tab-focus() {\n  // WebKit-specific. Other browsers will keep their default outline style.\n  // (Initially tried to also force default via `outline: initial`,\n  // but that seems to erroneously remove the outline in Firefox altogether.)\n  outline: 5px auto -webkit-focus-ring-color;\n  outline-offset: -2px;\n}\n"
...
var Strapping = window.Strapping = __webpack_require__(3);
